...
export const waitForDecodedFrames = async (page: Page, otherClientId: string) =>
  await test.step(`Wait for decoded frames`, async () => {
    return expectWithLongerTimeout.poll(async () => {
      const locator = await page.locator(`xpath=//div[@data-endpoint-id="${otherClientId}"]//span[@${DECODED_FRAMES}]`);
