...
export const onPeerJoined = <PeerMetadata, TrackMetadata>(peer: WebRtcPeer) =>
...
export const onPeerUpdated = <PeerMetadata, TrackMetadata>(peer: WebRtcPeer) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
    return {
      ...prevState,
      remote: {
...
export const onPeerLeft = <PeerMetadata, TrackMetadata>(peer: WebRtcPeer) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
    const remote: Record<PeerId, Peer<PeerMetadata, TrackMetadata>> = {
      ...prevState.remote,
    };
...
export const onPeerRemoved = <PeerMetadata, TrackMetadata>(reason: string) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
...
export const onTrackReady = <PeerMetadata, TrackMetadata>(ctx: TrackContext) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
...
export const onTrackAdded = <PeerMetadata, TrackMetadata>(ctx: TrackContext) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
    if (!ctx.peer) return prevState;
    if (!ctx.trackId) return prevState;
...
export const onTrackRemoved = <PeerMetadata, TrackMetadata>(ctx: TrackContext) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
    if (!ctx.peer) return prevState;
    if (!ctx.trackId) return prevState;
...
export const onTrackEncodingChanged = <PeerMetadata, TrackMetadata>(peerId: PeerId, trackId: TrackId, encoding: "l" | "m" | "h") =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
    const remote: Record<PeerId, Peer<PeerMetadata, TrackMetadata>> = {
      ...prevState.remote,
    };
...
export const onTrackUpdated = <PeerMetadata, TrackMetadata>(ctx: TrackContext) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
    const remote: Record<PeerId, Peer<PeerMetadata, TrackMetadata>> = {
      ...prevState.remote,
    };
...
export const onJoinSuccess = <PeerMetadata, TrackMetadata>(peersInRoom: [WebRtcPeer], peerId: PeerId, peerMetadata: PeerMetadata) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
...
export const addTrack = <PeerMetadata, TrackMetadata>(
...
export const replaceTrack = <PeerMetadata, TrackMetadata>(
  trackId: TrackId,
...
export const removeTrack = <PeerMetadata, TrackMetadata>(trackId: TrackId) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
    const prevLocalPeer = prevState.local
    if (!prevLocalPeer) return prevState;
...
export const updateTrackMetadata = <PeerMetadata, TrackMetadata>(trackId: TrackId, trackMetadata: TrackMetadata) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
    const prevLocalPeer = prevState.local
    if (!prevLocalPeer) return prevState;
...
export const onBandwidthEstimationChanged = <PeerMetadata, TrackMetadata>(estimation: bigint) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
    return {
      ...prevState,
...
export const onEncodingChanged = <PeerMetadata, TrackMetadata>(ctx: TrackContext) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
...
export const onVoiceActivityChanged = <PeerMetadata, TrackMetadata>(ctx: TrackContext) =>
  (prevState: State<PeerMetadata, TrackMetadata>): State<PeerMetadata, TrackMetadata> => {
